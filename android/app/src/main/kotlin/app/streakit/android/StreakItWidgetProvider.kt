...
class StreakItWidgetProvider : AppWidgetProvider() {
    override fun onUpdate(context: Context, appWidgetManager: AppWidgetManager, appWidgetIds: IntArray) {
        ...
        for (appWidgetId in appWidgetIds) {
            ...
            views.setTextViewText(R.id.widget_streak_label, if (checkedIn) "DONE TODAY" else "DAY STREAK")
...
class QuickCheckWidgetProvider : AppWidgetProvider() {
    override fun onUpdate(context: Context, appWidgetManager: AppWidgetManager, appWidgetIds: IntArray) {
        ...
        for (appWidgetId in appWidgetIds) {
            ...
            val summary = if (habitNames.isNotEmpty() && habitNames.first().isNotEmpty()) "$completed/${habitNames.size} done" else "No habits yet"
...
class ProgressRingWidgetProvider : AppWidgetProvider() {
    override fun onUpdate(context: Context, appWidgetManager: AppWidgetManager, appWidgetIds: IntArray) {
        ...
        for (appWidgetId in appWidgetIds) {
            ...
            views.setTextViewText(R.id.widget_progress_label, if (percentInt >= 100) "ALL DONE" else "DONE TODAY")
            val intent = Intent(context, MainActivity::class.java)
